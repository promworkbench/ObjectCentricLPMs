 ...
 * 
 */
public class OCLPMScalableViewPanel extends JLayeredPane implements Cleanable, ChangeListener, MouseMotionListener,
		UpdateListener {
	...
	private Map<OCLPMViewInteractionPanel, Integer> locations = new HashMap<OCLPMViewInteractionPanel, Integer>();
...
//		scroll.setBorder(new DashedBorder(theme.ELEMENTS));
	 ...
	 */
	public synchronized void removeViewInteractionPanel(OCLPMViewInteractionPanel panel) {
	 ...
	 */
	public Map<OCLPMViewInteractionPanel, Integer> getViewInteractionPanels() {
		return new HashMap<OCLPMViewInteractionPanel, Integer>(locations);
	}
	...
	public synchronized void mouseMoved(MouseEvent e) {
		...
		if (c == this || isChild(c, getComponent())) {
			...
			for (Entry<OCLPMViewInteractionPanel, Pair<JPanel, JPanel>> entry : panels.entrySet()) {
				JPanel panelOn = entry.getValue().getFirst();
				JPanel panelOff = entry.getValue().getSecond();
				OCLPMViewInteractionPanel panel = entry.getKey();
...
//		JButton button = factory.createButton("");
	...
	public void setSize(OCLPMViewInteractionPanel panel, JPanel panelOff, JPanel panelOn) {
	...
	public void setLocation(OCLPMViewInteractionPanel panel, JPanel panelOff, JPanel panelOn) {
	...
	public void updated() {
		...
		for (OCLPMViewInteractionPanel panel : panels.keySet()) {
